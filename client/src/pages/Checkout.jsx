...
export default function Checkout(){
  ...
  async function placeOrder(){
    ...
    if (cErr) {
      setStatus({ type:"err", msg: cErr, orderId:"" });
      setStep(1);
      return;
    }
    ...
    if (aErr) {
      setStatus({ type:"err", msg: aErr, orderId:"" });
      setStep(2);
      return;
    }
          ...
          {step===1 && (
            <div className="grid" style={{ gap: 12 }}>
              ...
              {status.type==="err" && status.msg && (
                <div className="card" style={{ padding:"12px 14px", outline:"1px solid rgba(255,91,91,.35)" }}>
                  <div style={{ fontWeight:900, color:"var(--danger)" }}>{status.msg}</div>
                </div>
              )}
              ...
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                ...
                <button
                  ...
                  onClick={()=>{
                    ...
                    if (err) {
                      setStatus({ type:"err", msg: err, orderId:"" });
                      return;
                    }
                    setStatus({ type:"", msg:"", orderId:"" });
                    setStep(2);
            ...
            <div className="grid" style={{ gap: 12 }}>
              <div className="card" style={{ padding: 14, background:"rgba(0,0,0,.18)" }}>
                ...
                <div style={{ display:"flex", gap: 10, marginTop: 12, flexWrap:"wrap" }}>
                  <button className={"btn " + (payment.method==="COD" ? "primary":"ghost")}
                          onClick={()=>setPayment({ method:"COD" })}>Cash on Delivery</button>
                  <button className={"btn " + (payment.method==="UPI" ? "primary":"ghost")}
                          onClick={()=>setPayment({ method:"UPI" })}>UPI</button>
                </div>
              </div>
